...
func SetCredentials(uri *url.URL, secretProvider interfaces.SecretProvider, category string, authMode string, secretName string, certkeypair string) error {
	switch authMode {
	case AuthModeCertificate:
		return nil
		// credentials, err := GetCertKeyPair(secretProvider, certkeypair)
		// if err != nil {
		// 	return fmt.Errorf("Unable to get %s MQTT credentials for secret name '%s': %s", category, certkeypair, err.Error())
		// }
...
func GetCertKeyPair(secretProvider interfaces.SecretProvider, certkeypair string) (config.CertKeyPair, error) {
	...
	for timer.HasNotElapsed() {
		secretData, err = secretProvider.GetSecret(certkeypair, "certificate", "key")
		if err == nil {
			break
		}

		driver.Logger.Warnf(
			"Unable to retrieve MQTT cert key pair from SecretProvider at secret name '%s': %s. Retrying for %s",
			certkeypair,
			err.Error(),
	...
	certKeyPair.Cert = secretData["certificate"]
	certKeyPair.Key = secretData["key"]
